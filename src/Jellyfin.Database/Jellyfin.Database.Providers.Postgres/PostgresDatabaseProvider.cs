...
{
    private const string BackupFolderName = "PostgresBackups";
    ...
    private NpgsqlConnectionStringBuilder GetConnectionBuilder(ICollection<CustomDatabaseOption>? options)
    {
        ...
        {
            Host = Environment.GetEnvironmentVariable("JELLYFIN_POSTGRES_HOST") ?? "localhost",
            Port = int.Parse(Environment.GetEnvironmentVariable("JELLYFIN_POSTGRES_PORT") ?? "5432", CultureInfo.InvariantCulture),
            Database = Environment.GetEnvironmentVariable("JELLYFIN_POSTGRES_DATABASE") ?? "jellyfin",
            Username = Environment.GetEnvironmentVariable("JELLYFIN_POSTGRES_USERNAME") ?? "jellyfin",
            Password = Environment.GetEnvironmentVariable("JELLYFIN_POSTGRES_PASSWORD") ?? throw new InvalidOperationException("PostgreSQL password must be provided via JELLYFIN_POSTGRES_PASSWORD environment variable")
        };
