...
# Scan all tracked .cs and .csproj files for unresolved merge markers.
# git apply --3way can leave <<<<<<< / ======= / >>>>>>> in files,
# which will cause build errors (CS8300: Merge conflict marker encountered).
CONFLICT_FILES=$(grep -rlE '<<<<<<<|>>>>>>>|=======' --include='*.cs' --include='*.csproj' . 2>/dev/null || true)
...
if [ -n "$CONFLICT_FILES" ]; then
    ...
    echo ""
    echo "These markers will cause build errors (CS8300)."
